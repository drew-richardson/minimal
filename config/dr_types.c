...
int main(void) {
  static const char buf[] = {
    ...
    ' ','_','_','p','r','i','v','a','t','e','[',
    DR_HEXSTR((SIZE(dr_event_impl_t) + ALIGN(dr_event_impl_t) - 1)/ALIGN(dr_event_impl_t)),
    ']',';',' ','}',';','\n',

    's','t','r','u','c','t',' ','d','r','_','e','q','u','e','u','e',' ','{',' ',
    DR_XINTXX2(ALIGN(struct dr_equeue_impl), 1),
    ' ','_','_','p','r','i','v','a','t','e','[',
    DR_HEXSTR((SIZE(struct dr_equeue_impl) + ALIGN(struct dr_equeue_impl) - 1)/ALIGN(struct dr_equeue_impl)),
    ']',';',' ','}',';','\n',

    's','t','r','u','c','t',' ','d','r','_','e','q','u','e','u','e','_','s','e','r','v','e','r',' ','{',' ',
    DR_XINTXX2(ALIGN(struct dr_equeue_server_impl), 1),
    ' ','_','_','p','r','i','v','a','t','e','[',
    DR_HEXSTR((SIZE(struct dr_equeue_server_impl) + ALIGN(struct dr_equeue_server_impl) - 1)/ALIGN(struct dr_equeue_server_impl)),
    ']',';',' ','}',';','\n',

    's','t','r','u','c','t',' ','d','r','_','e','q','u','e','u','e','_','c','l','i','e','n','t',' ','{',' ',
    DR_XINTXX2(ALIGN(struct dr_equeue_client_impl), 1),
    ' ','_','_','p','r','i','v','a','t','e','[',
    DR_HEXSTR((SIZE(struct dr_equeue_client_impl) + ALIGN(struct dr_equeue_client_impl) - 1)/ALIGN(struct dr_equeue_client_impl)),
    ']',';',' ','}',';','\n',
